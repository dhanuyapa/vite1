...
// Assuming you have a HeaderMain component
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        if (nic === '888888888888' && password === '88888888@') {
          navigate('/addd'); // Navigate to hfetch page
        } else {
         ...
         <div className="form">
          ...
          {loginSuccess ? (
            ...
            <form onSubmit={handleSubmit}>
              ...
              <center>
                <Link to="/add" className="signup-link-yapa">
                  Sign up
                </Link>
              </center>
            </form>
          )}
          {errorMessage && <p className="error-message-yapa">{errorMessage}</p>}</div>
        </div>
